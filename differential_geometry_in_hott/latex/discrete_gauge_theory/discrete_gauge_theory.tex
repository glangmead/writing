...
\begin{mylemma}
\label{lem:verthoriz}
(\cite{Symmetry} Lemma 4.14.1). Given 
\begin{itemize}
\item a type family \( f:X\to\uni \)
\item terms \( x,x':X \)
\item a section \( s:\pit{x:X}f(x) \) 
\end{itemize}
then there is an equivalence 
\[ 
\left((x,f(x)) =_{\sit{x:X}f(x)} (x',f(x'))\right) \simeq 
(x=x')\times (f(x)=f(x)).
\]
\end{mylemma}
\begin{proof}
...
\[ 
\sit{p:x=x'}f(x)\pathover{p}f(x') \simeq \sit{p:x=x'}f(x)=f(x).
\] 
This reduces to proving \( f(x)\pathover{p}f(x')\equiv f(x)=f(x)  \). By induction when \( p=\refl \) both sides are \( f(x)=f(x) \) which we prove with \( refl \).
\end{proof}

One thing the equivalence in Lemma~\ref{lem:verthoriz} says is that in the presende of a section, and given a path in the base, the dependent paths over that path are equivalent to loops at one upstairs endpoint (the section applied to a downstairs endpoint).

Note also that in Lemma~\ref{lem:verthoriz} we are expressing the type of pathovers (the left hand side) as paths in the sigma type of the type family. The sigma type comes equipped with a projection \( \pr_1:\sit{x:X}f(x)\to X \) and since \( \pr_1\circ f\equiv\id \) we have that \( \ap(\pr_1) \) maps the pathover back to the downstairs path, confirming the ``over'' portion of the name.
...
\subsection{Deconstruct a map to the delooping of a group}

A map \( B\dotto \BAuto{G} \) is 
