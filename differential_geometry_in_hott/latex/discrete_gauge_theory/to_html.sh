...
do
  ...
  cd tmp
  cp ../greg.sty ../agda.sty ../quiver.sty ../$i.yml ../aims-mathematics.csl ../connections.bib ../tikz.py ../pandoc_only_macros.tex .
  ...
  ../pandoc-3.1.13-arm64/bin/pandoc --wrap=none --from latex+latex_macros --mathjax \
    ...
    pandoc_only_macros.tex $i.tikzimg.tex -t html -o ../html/$i.html
  cp -f *.png *.jpg *.svg ../html || :
  cd ..
  cp -f *.png *.jpg *.svg html || :
  cp -f figs/*.png figs/*.jpg fits/*.svg html || :
