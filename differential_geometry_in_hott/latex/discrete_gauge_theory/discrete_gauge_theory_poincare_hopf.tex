...
When the fibers are pointed then loops are pointed automorphisms of mere circles, which are integers.

\subsection{Equality of total index and total curvature}

When we use the dependent action on paths, we obtain paths between the transport from the connection and the swirling of the vector field, and these cancel.

\subsection{Identification with Euler characteristic}

Combinatorial manifolds are intuitive objects that connect directly to the classical definition of Euler characteristic.
