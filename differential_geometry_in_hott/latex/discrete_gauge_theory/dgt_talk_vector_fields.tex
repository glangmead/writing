...
\begin{frame}{Index}
\[\begin{aligned}
\tr_F&\defeq \tr(\partial F)&&:T_1=_{\Kzt}T_1&&\text{\alert{curvature}}\\
...
\begin{column}{0.7\textwidth}
\begin{mydef}
The \defemph{index of the vector field \( X \) on the face \( F \)} is the integer \[ I^X_F\defeq\alert{\loopy(\flat_F(X_1)\cdot X_F)}:\loopy(X_1=_{T_1}X_1). \]
\end{mydef}
\end{column}
\begin{column}{0.3\textwidth}
\begin{tikzpicture}
