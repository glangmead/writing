...
\end{tikzcd}\]
\onslide<2->{\input{figs/discrete_gauge_theory_oo_swirl}}
\end{column}
\end{columns}
\end{frame}
...
\input{figs/face_swirl_bare_tikz.tex}
\endgroup
\end{column}
\begin{column}{0.35\textwidth}
...
\begingroup
\tikzset{every picture/.style={scale=0.85}}
\input{figs/face_swirl_singlevec4_tikz.tex}
\endgroup
\end{column}
\begin{column}{0.35\textwidth}
An example of \alert{swirling} and \alert{index} at this face.
...
\end{aligned}\]
\onslide<3->{({\itshape Recall that \( T_1 \) being an \( S^1 \)-torsor means we can use subtraction to obtain an equivalence \( s(-,X_1):T_1\xrightarrow[]{x\mapsto x\alert{-X_1}} S^1 \).})}
\onslide<2->{
\begin{columns}[c]
\begin{column}{0.7\textwidth}
\begin{mydef}
