...
\item<3-> \( X \) is a \alert{torsor} if it is inhabited and the map \( (\phi, \pr_1):X\times G\to X\times X \) is an equivalence.
\item<4-> Let \( \reg{G} \) be the \( G \)-torsor consisting of \( G \) acting on itself on the right.
\end{itemize}
\end{definition}}
\end{frame}
