...
First we will choose a specific group that acts on all the fibers of \( T \). Fix a point \( m:\mm \), and recall the map \( \tau:\pit{x:M}((Tx=Tx)\xrightarrow[]{\simeq}(Tm=Tm)) \) from Lemma~\ref{lem:gauge_triv}. This map is among other things an action of \( Tm=Tm \) on each \( Tx \) (or in our case \( T_i \)), so that is our choice. Denote the action of \( Tm=Tm \) on the torsor \( T_i \) by \( \rho:(Tm=Tm)\times T_i\to T_i \). This induces an equivalence \( (\rho, \pr_2):(Tm=Tm)\times T_i\simto T_i\times T_i \).

\begin{mydef}
The map \( \pr_1\circ (\rho, \pr_2)^{-1}:T_i\times T_i\to (Tm=Tm) \) is called \defemph{subtraction}. It maps \( (x,y) \) to the unique term \( \alpha:Tm=Tm \) such that \( \rho(\alpha, x)=y \). For brevity we denote \( \pr_1\circ (\rho, \pr_2)^{-1}(x,y) \) by \( y-x \).
\end{mydef}

\begin{mylemma} The following diagram is well-typed and commutes, and therefore \( \rho_{ij}=-\rho_{ji} \).
\[% https://q.uiver.app/#q=
\begin{tikzcd}
...
\end{mylemma}
\begin{proof}
The top map has the given codomain because \( T_{ji}T_{ij}=\id \). To prove commutativity we need \( -X_j\circ T_{ji}=+\rho_{ji} \).
The proof of commutativity of the right-hand square of types shows that the maps \( -X_i \) and \( -X_j \) map pathovers to paths with endpoints that are inverses in \( Tm=Tm \), i.e. that \( p_{ij}=-p_{ji} \). 
\end{proof}

Given this fact we now know that \( -X_j \) maps \( X_{ij}\cdot X_{ji} \) to \( \id=\id \), and we can ask if the image is \( \refl \). We have
\begin{alignat*}{2}
