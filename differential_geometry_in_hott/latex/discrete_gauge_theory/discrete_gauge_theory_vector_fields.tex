...
Vector fields are sections of the tangent bundle of a manifold. We do not have a general theory of tangent bundles, even for 2-dimensional cellular types, since we cannot yet prove that connections always exist on the 1-skeleton. But if \( \mm\defeq\mm_o\to\mm_1\to\mm_1 \) is a cellular type, and \emph{given} an extension \( T:\mm\to\EMzo \) of the \( \link \) function, we can consider the type of sections \( \pit{x:\mm_1}T_1(x) \).

In this section and for the remainder of the note, we will assume that the bundle is oriented, that is that the bundle is in fact a map \( T:\mm\to\Kzt \), the type of \( S^1 \)-torsors.

\begin{mydef}
Given a 2-dimensional cellular type \( \mm\defeq\mm_o\to\mm_1\to\mm_1 \) equipped with type family \( T:\mm\to\Kzt \) extending \( \link \), a \defemph{vector field} on \( \mm \) is a term \( X:\pit{x:\mm_1}T_1(x) \).
\end{mydef}

\begin{mynote}
The circle bundle extending \( \link \) captures the \emph{unit spheres} of the classical tangent bundle. A section of this bundle is therefore analogous to a classical \emph{nonvanishing} vector field. To allow for classical vector fields \emph{with zeroes}, we are limiting the section \( X \) to the 1-skeleton of \( \mm \).
\end{mynote}
...
We need to derive two operations from the action \( \Gg(m) \).
...
\begin{mylemma}
\label{lem:multpath}
If \( G \) is a higher group with multiplication \( \mu:G\times G\to G \) and proof of commutativity \( \mathsf{is\_comm}:\pit{a,b:G}\mu(a, b)=\mu(b, a) \) then \( \mu \) induces a function \( \mu_=:(x=_G y)\times (x'=_G y')\to (\mu(x, x')=_G\mu(y,y')) \).
\end{mylemma}
\begin{myproof}
...
Each fiber \( T_i \) is pointed by \( X_i \), so we can define the map \( T_i\xrightarrow[]{-X_i}\Gg(m) \), and then give a name to the special term \( \rho_{ij}\defeq T_{ij}X_j - X_i \). 
...
\end{align*}
\end{myproof}
Now we want to make use of the fact that dependent functions such as our vector field send \( \refl_{v_i} \) to \( \refl_{X_i} \) (by path induction, see for example \cite{hottbook} Lemma 2.3.4), but mapped from \( X_i=_{T_i}X_i \) to the non-dependent type \( 0=_{\Gg(m)}0 \).

We can obtain a path in \( \Gg(m) \) from a dependent path by again subtracing the basepoint, e.g. \( \sigma_{ij}\defeq X_{ij}-X_j:0=_{\Gg(m)}T_{ji}X_i-X_j \), which type also has the name \( 0=_{\Gg(m)}\rho_{ji} \). Notice the reversal in indices between \( \sigma_{ij} \) and \( \rho_{ji} \), which reflects our opposite conventions for \( X_{ij} \) and \( T_{ji} \).
...
\begin{align*}
\sigma_{ij}&:0=_{\Gg(m)}\rho_{ji}\\
\sigma_{ji}&:0=_{\Gg(m)}\rho_{ij}
\end{align*}
so when we add these paths with \( + \) we obtain a path \( \sigma_{ij}+\sigma_{ji}:0=_{\Gg(m)} \rho_{ji}+\rho_{ij} \) which by Lemma~\ref{lem:subtraction} is \( 0=_{\Gg(m)}0 \). We compute \( + \) using the concatenations in Lemma~\ref{lem:multpath}, which gives
\begin{align*}
\sigma_{ij}+\sigma_{ji} &: (0+0) \overset{\sigma_{ij}}{=\joinrell=} (0+\rho_{ji}) \overset{\mathsf{is\_comm}}{=\joinrell=}(\rho_{ji}+0) \overset{\rho_{ji}+\sigma_{ji}}{=\joinrell=}(\rho_{ji}+\rho_{ij})\,\,\big[\overset{\mathrm{Lemma~\ref{lem:subtraction}}}{=\joinrell=}0\big]\\
\sigma_{ij}+\sigma_{ji}&=_{0=_{\Gg(m)}0} (X_{ij}-X_j)\cdot (\rho_{ji} + (X_{ji} - X_i))\\
&=_{0=_{\Gg(m)}0} (X_{ij}-X_j)\cdot (T_{ji}(X_{ji} - X_i))\\
&=_{0=_{\Gg(m)}0} \\
&=_{0=_{\Gg(m)}0} \refl_0
\end{align*}%
\end{myproof}%
