...
\begin{mydef}
\label{def:curvature}
The \defemph{curvature} on a bundle-with-connection \( f_1:\mm_1\to\uni \) at a vertex \( v \) of a 2-face \( F \) with boundary path \( p_F \) is the automorphism \( \tr(p_F)(fv) \). An extension of \( f_1 \) to \( F \) is a homotopy \( H_F:\tr(p_F)(fv)\sim\id_{fv} \) from the curvature of \( F \) at \( v \) to the identity. We will call such an extension a \defemph{flatness structure on the face \( f \)}. By analogy to the definition of a 1-connection we can also call this a \defemph{2-connection}.
\end{mydef}

For a 2-connection to exist on a face \( F \), the holonomy map \( \tr(p_F) \) must be in the connected component of the identity map \( \id_{fv} \).
