...
\begin{tikzpicture}[arrow/.style={-{Stealth[scale=1.1]}}]
  \tikzset{internal/.style={inner sep=1, outer sep=1, line cap=rect}}
    % Define coordinates for the three additional vertices on the line segment
    \node[internal, label=below:\( \begin{array}{c} f(p_i)\\g(p_i)\end{array} \)]  (P1)  at (-6, 0) {};
    \node[internal, label=below:\( f(p_{i+1}) \)]  (P21) at (-4, 0) {};
    \node[internal, label=above:\( g(p_{i+1}) \)] (P22) at (-4, 2) {};
    \node[internal, label=below:\( \begin{array}{c} f(p_{i+2})\\g(p_{i+2})\end{array} \)]  (P3)  at (-2, 0) {};

    % f
    \draw[arrow] (P1)  edge["\( f(e_i) \)"] (P21);
    \draw[arrow] (P21) edge["\( f(e_{i+1})\)"]  (P3);
